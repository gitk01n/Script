...
注: 如需清除所有Cookie, 您可开启脚本内"DeleteCookie"选项 
...
# 获取京东Cookie. 
# 注意此为远程路径, 低版本用户请自行调整为本地路径.
https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean url script-request-header https://raw.githubusercontent.com/gitk01n/Script/main/JD_DailyBonus.js
...
function notify() {
  return new Promise(resolve => {
    try {
      ...
      const disa = !notify || amount ? `【温馨提示】:  检测到${$nobyda.disable?`上次执行意外崩溃, `:``}已禁用${notify?`${amount}个`:`所有`}接口, 如需开启请前往BoxJs或查看脚本内第75行注释.\n` : ``
